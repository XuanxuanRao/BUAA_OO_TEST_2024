...
def diff(out1, out2) -> int:
    with open('out\\' + out1, 'r') as file1, open('out\\' + out2, 'r') as file2:
        ...
        file1.seek(0)
        file2.seek(0)
        file_length1 = len(list(file1))
        file_length2 = len(list(file2))
        if file_length1 != file_length2:
            s = out2[out2.find('_') + 1:-4]
            with open(f'record\\{out1[:-4]}_{s}.txt', 'w') as record_file:
                record_file.write(f'Output lengths differ, {out1}: {file_length1}, {out2}: {file_length2}')
                return min(file_length1, file_length2) + 1
        else:
            return -1
