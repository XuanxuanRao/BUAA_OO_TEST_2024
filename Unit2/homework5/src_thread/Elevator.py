...
class Elevator:
    def __init__(self, elevator_id: int, floor=1):
        self.floor = floor
        self.requests = []
        self.is_open = False
        self.capacity = 6
    ...
    def move(self, floor, timestamp: str) -> bool:
        if (self.is_open or Decimal(timestamp) - Decimal(self.last_move_time) < Decimal(move_time)
                or floor <= 0 or floor >= 12 or math.fabs(floor - self.floor) != 1 or
                Decimal(timestamp) - Decimal(self.last_close_time) < Decimal(move_time)):
            return False
        self.floor = floor
